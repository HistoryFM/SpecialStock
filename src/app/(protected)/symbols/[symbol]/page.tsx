        ...
        <div className="audit-grid">
          ...
          <aside className="audit-metadata">
            ...
            <dl>
              ...
              <div><dt>Inference profile</dt><dd><code>{selected.run.inferenceProfile}</code></dd></div>
              ...
              <div><dt>Reasoning / output tokens</dt><dd>{selected.run.reasoningTokens?.toLocaleString() ?? "unknown"} / {selected.run.outputTokens?.toLocaleString() ?? "unknown"}</dd></div>
              <div><dt>Provider queue wait</dt><dd>{selected.run.queueWaitMs.toLocaleString()} ms</dd></div>
            </dl>
            <details className="prompt-audit"><summary>Compact provider attempts</summary>{data.promptAudit.compact.attempts.length ? data.promptAudit.compact.attempts.map((attempt) => <div key={attempt.attemptNumber}><p><strong>Attempt {attempt.attemptNumber}</strong> · {attempt.status} · provider {attempt.latencyMs.toLocaleString()} ms · queued {attempt.queueWaitMs.toLocaleString()} ms</p><p>Reasoning / output tokens: {attempt.reasoningTokens?.toLocaleString() ?? "unknown"} / {attempt.outputTokens?.toLocaleString() ?? "unknown"} · cost: ${Number(attempt.costUsd ?? attempt.estimatedCostUsd ?? 0).toFixed(6)}{attempt.costUsd === null && attempt.estimatedCostUsd !== null ? " estimated" : ""}{attempt.failureKind ? ` · ${attempt.failureKind}` : ""}</p><pre>{JSON.stringify(attempt.requestSettings, null, 2)}</pre></div>) : <p className="muted">Legacy run without attempt telemetry.</p>}</details>
            {data.promptAudit.full.run ? <details className="prompt-audit"><summary>Full-analysis provider attempts</summary>{data.promptAudit.full.attempts.length ? data.promptAudit.full.attempts.map((attempt) => <div key={attempt.attemptNumber}><p><strong>Attempt {attempt.attemptNumber}</strong> · {attempt.status} · provider {attempt.latencyMs.toLocaleString()} ms · queued {attempt.queueWaitMs.toLocaleString()} ms</p><p>Reasoning / output tokens: {attempt.reasoningTokens?.toLocaleString() ?? "unknown"} / {attempt.outputTokens?.toLocaleString() ?? "unknown"} · cost: ${Number(attempt.costUsd ?? attempt.estimatedCostUsd ?? 0).toFixed(6)}{attempt.costUsd === null && attempt.estimatedCostUsd !== null ? " estimated" : ""}{attempt.failureKind ? ` · ${attempt.failureKind}` : ""}</p><pre>{JSON.stringify(attempt.requestSettings, null, 2)}</pre></div>) : <p className="muted">Legacy run without attempt telemetry.</p>}</details> : null}
