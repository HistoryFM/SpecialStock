...
export async function POST(request: Request) {
  if (!isAuthorizedSession(await auth())) return Response.json({ error: "Unauthorized" }, { status: 401 });
  try {
    const body = z.object({ mode: z.enum(["manual", "automatic"]), requestId: z.string().uuid().optional() }).strict().parse(await request.json());
    const created = await createSwingRun(body);
    if (created.run.status === "scheduled") void executeSwingRun(created.run.id);
