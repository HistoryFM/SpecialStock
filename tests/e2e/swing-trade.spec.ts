...
test("versions Swing inputs, runs a macro-first mocked batch, and audits exact artifacts", async ({ page }) => {
  ...
  await expect(page.getByText("4 stocks · version 2")).toBeVisible();
  await page.getByText("Watchlist version history").click();
  await page.locator(".swing-history-list > div").filter({ hasText: "v1" }).getByRole("button", { name: "Restore" }).click();
  await expect(page.getByText("2 stocks · version 1")).toBeVisible();
