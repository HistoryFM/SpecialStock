...
Open **Swing Trade** after signing in. Import a separate CSV or XLSX watchlist with case-insensitive `Stock Name`, `Symbol`, and `Exchange` headers. It accepts 1–100 ordered US symbols from NASDAQ, NYSE, or AMEX and normalizes `NYSE American` to AMEX. Files with more than 20 valid rows open a searchable, initially unselected checkbox list. **Select first 20**, **Select all**, and individual choices can activate up to 100 stocks while preserving workbook order; a source with more than 100 rows requires choosing at most 100. Import validation is atomic: an invalid row leaves the active version unchanged. XLSX workbooks must contain exactly one populated worksheet. Previous immutable versions can be restored.
