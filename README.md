...
Open **Backtesting** after signing in. Import daily CSVs for every signal or traded ticker, plus SPY and QQQ. Each upload must contain `Date` and `Close/Last` or `Close`. The import accepts the supplied `Date,Close/Last,Volume,Open,High,Low` format, sorts dates, rejects duplicates and invalid closes, and shows date ranges, row counts, and warnings. Confirm that close prices are split-adjusted before import. Each imported version remains under `.data/backtesting/files/` for reproducible saved runs; data is not forwarded to the scan pipeline.

Describe the entire portfolio in one prompt: starting capital, cash interest, short-borrow rate, fees, slippage, start date, signal assets, phase transitions, target allocations, and optional position stops. The Backtesting-only model selector offers Gemini 2.5 Pro, GPT-5.6 Sol High, and Claude Opus 5 High through the existing server-side OpenRouter key. AI returns a versioned, reviewable rule plan; it does not generate executable Python. The confirmation panel shows exact asset-specific SMA 50/200, RSI, MACD, percentage-band, and crossover conditions, ordered phases, allocations, stops, and visible numeric defaults. Unsupported or ambiguous rules require clarification. A missed crossover is not replayed on later days.
...
Annual rows show strategy and asset price returns; a separate table gives one full-period maximum drawdown per series. The growth chart has year labels for the full period and month labels for shorter views. Trade rows show the traded close and SPY/QQQ closes, with optional other imported close columns. Report sections, growth series, price columns, and chart range can be changed manually or through a separate AI report prompt; each saved run retains its configuration. AI commentary uses calculated metrics and may suggest untested plans, which run only when **Test suggestion** is clicked and are compared with the original over identical dates and CSV versions. Older saved runs keep their original calculations and explicit rules, including any 200-day SMA filter.
